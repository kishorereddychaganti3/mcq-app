...
for (let e of examData || []) {
 ...
 async function saveMapping() {
  ...
  try {
    ...
    const existingIds = new Set(existing?.map(x => x.question_id))

    const newToInsert = selectedQuestions
    ...
    if (newToInsert.length > 0) {
      await supabase.from('exam_questions').insert(newToInsert)
    }
    ...
    setSuccessMsg('✅ Questions mapped successfully!')
    await fetchExams()
