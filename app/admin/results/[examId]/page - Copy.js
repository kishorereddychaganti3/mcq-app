...
students?.forEach(s => {
  ...
  const leaderboard = Object.values(
    submitted.reduce((acc, s) => {
      ...
      acc[s.student_id].best = Math.max(acc[s.student_id].best, s.score || 0)
      return acc
    }, {})
  ...
  submitted.forEach(s => {
    const totalQ = Object.keys(s.answers || {}).length
    ...
    {activeTab === 'overview' && (
      <div style={{
        ...
        <div style={kpiCard}>
          <div style={kpiNumber}>
            {submitted.length > 0
              ? (submitted.reduce((a,b)=>a+(b.score||0),0)/submitted.length).toFixed(1)
              : 0}
          </div>
...
<tbody>
  {leaderboard.map((l,i)=>(
    <tr key={l.student_id}>
      ...
      <td style={td}>{studentsMap[l.student_id]?.name}</td>
      <td style={{...td, color:'#6b7280'}}>
        {studentsMap[l.student_id]?.email}
      </td>
